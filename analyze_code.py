...
def _build_java_project(dir_path):
    ...
    if os.path.exists(pom_path):
        ...
        proc = subprocess.Popen(
            [r'mvn', 'clean', 'install', '-DskipTests'], shell=True if platform == 'win32' else False)
        proc.wait()
        is_compiled = True
        os.chdir(orig_path)
    ...
    if os.path.exists(gradle_path):
        ...
        os.chdir(dir_path)
        try:
            proc = subprocess.Popen([r'gradle', 'compileJava'], shell=True if platform == 'win32' else False)
            proc.wait()
...
def _run_designite_java(folder_path, out_path, designite_path):
    ...
    proc = subprocess.Popen(
        ...
        shell=True if platform == 'win32' else False)
    proc.wait()
    print("done analyzing.")


def _run_designite_java_multi(folder_path, out_path, designite_path, branch_name):
    print("Analyzing Multi...")
    proc = subprocess.run(
        ["java", "-jar", designite_path, "-i", folder_path, "-o", out_path, "-aco", branch_name], shell=True if platform == 'win32' else False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def analyze_repo_multi(folder_path, result_path, designite_path, branch_name):
    print("Analyzing" + folder_path + " ...")

    _build_java_project(folder_path)
    _run_designite_java_multi(folder_path, result_path, designite_path, branch_name)
